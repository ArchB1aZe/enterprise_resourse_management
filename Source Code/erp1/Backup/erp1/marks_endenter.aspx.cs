...
namespace erp1
{
    public partial class marks_endenter : System.Web.UI.Page
    {
        String a, b, c, d;
        int count;
        DataSet ds;
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            for (int ll = 0; ll < count; ll++)
            {
                ...
                {
                    int check = 1;
                    Response.Redirect("marks_endenter.aspx?branch=" + a + "&sem=" + b + "&sub=" + c + "&scode=" + d + "&check=" + check + "");
                }
                if (Convert.ToInt32(tb11[ll].Text) > 60 || Convert.ToInt32(tb11[ll].Text) < 0)
                {
                    int check = 1;
                    Response.Redirect("marks_endenter.aspx?branch=" + a + "&sem=" + b + "&sub=" + c + "&scode=" + d + "&check=" + check + "");
                }
            ...
            for (int j = 0; j < count; j++)
            {
                ...
                {
                    SqlDataAdapter ad1 = new SqlDataAdapter("update marks set endsem='" + tb11[j].Text + "' where rno='" + ds.Tables[0].Rows[j][0] + "' and scode='" + d + "'", "server=B1aZe;database=erp;integrated security=true");
                    DataSet ds2 = new DataSet();
                    ad1.Fill(ds2);
                }
                else
                {
                    SqlDataAdapter ad2 = new SqlDataAdapter("insert into marks values('" + ds.Tables[0].Rows[j][0] + "','NULL','" + tb11[j].Text + "','NULL','" + d + "','" + ds.Tables[0].Rows[j][3] + "')", "server=B1aZe;database=erp;integrated security=true");
