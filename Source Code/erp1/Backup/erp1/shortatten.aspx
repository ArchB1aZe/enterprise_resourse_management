<%@ Page Title="" Language="C#" MasterPageFile="~/Aveg.Master" AutoEventWireup="true" CodeBehind="shortatten.aspx.cs" Inherits="erp1.shortatten" %>
 ...
 <table style="border-style: none; border-color: inherit; border-width: 0px; width: 1200px; margin-bottom:10px; border:2px solid navy; ">
    ...
    <tr style="border:0px; font-size:larger">
     ...
     <asp:Panel ID="Panel1" runat="server" ScrollBars="Vertical" Wrap="True" Height="390px">
        ...
        <td style="border-color:Black; border-width:2px; border-style:solid; font-weight:bold; color:Black;" width="100px">S.No.</td>
            
        <td style="border-color:Navy; border-width:2px; border-style:solid; font-weight:bold; color:Navy;" width="500px">Student Name</td>
        <td style="border-color:Navy; border-width:2px; border-style:solid; font-weight:bold; color:Navy;" width="500px">Roll Number</td>
      ...
      DataSet ds1 = new DataSet();
       ad.Fill(ds1);
       SqlDataAdapter adx = new SqlDataAdapter("select * from attendance where scode='" + d + "' and extra='YES'", "server=B1aZe;database=erp;integrated security=true");
      ...
      DataSet ds = new DataSet();
       ...
       for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
       {
           ...
           for (int k = 0; k < dsx.Tables[0].Rows.Count; k++)
           {

               if (dsx.Tables[0].Rows[k][1].ToString() == ds.Tables[0].Rows[i][0].ToString())
               {
                   extrast = extrast + Convert.ToInt32(dsx.Tables[0].Rows[k][3]);
           ...
           if ((Math.Round(((atten+extrastd) / (total+extrastd)), 4) * 100) < 75.00)
           {
        %>
        <tr style="font-size:large; color:Black; font-weight:bold; border-color:Black; border-style:solid; border-width:2px;">
        <td style="border-color:Black; border-width:2px; border-style:solid; font-weight:bold; color:White;" width="100px"><%=m%></td>
            
        <td style="border-color:Black; border-width:2px; border-style:solid; font-weight:bold; color:White;" width="500px"><%=ds.Tables[0].Rows[i][1]%></td>
        <td style="border-color:Black; border-width:2px; border-style:solid; font-weight:bold; color:White;" width="500px"><%=ds.Tables[0].Rows[i][0]%></td>
        
        <td style="border-color:Black; border-width:2px; border-style:solid; font-weight:bold; color:White;" width="100px"><%=(Math.Round(((atten+extrastd) / (total+extrastd)), 4) * 100)%>%</td>
        </tr>
